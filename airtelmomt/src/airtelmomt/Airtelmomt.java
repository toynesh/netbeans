...
public class Airtelmomt {
    ...
    static public void main(String[] args) throws Exception {
        ...
        config0.setHost("41.223.59.92");
        config0.setPort(8188);
        config0.setConnectTimeout(10000);
        config0.setSystemId("PDSLKeMoMt");
        config0.setPassword("pdsl@997");
        //config0.setPassword("pdsl@2019");
        ...
        try {
            ...
            t.schedule(new TimerTask() {
                ...
                public void run() {
                    try {
                        ...
                        //Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE,
                        EnquireLinkResp enquireLinkResp1 = session0.enquireLink(new EnquireLink(), 10000);
                        Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "enquire_link_resp #1: commandStatus [" + enquireLinkResp1.getCommandStatus() + "=" + enquireLinkResp1.getResultMessage() + "]");
                        if (session0 == null) {
                        ...
                        if (!future0.await()) {
                            Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE,"Failed to receive enquire_link_resp within specified time");
                        } else if (future0.isSuccess()) {
                            EnquireLinkResp enquireLinkResp2 = (EnquireLinkResp) future0.getResponse();
                            Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO,"enquire_link_resp #2: commandStatus [" + enquireLinkResp2.getCommandStatus() + "=" + enquireLinkResp2.getResultMessage() + "]");
                        } else {
                            Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE,"Failed to properly receive enquire_link_resp: " + future0.getCause());
                        }*/
                    } catch (RecoverablePduException ex) {
                        java.util.logging.Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE, null, ex);
            ...
            for (;;) {
                try {
                    con = data.connect();
                    String query = "SELECT snd_id, snd_sender, snd_to,snd_txt FROM snd where  snd_smsc='AIRTELRX' and status=0 order by snd_id desc";
                    ...
                    while (rs.next()) {
                        ...
                        Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "sendWindow.size: {}", session0.getSendWindow().getSize());
            ...
            Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE, "", e);
        }

        if (session0 != null) {
            Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "Cleaning up session... (final counters)");
            if (session0.hasCounters()) {
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "tx-enquireLink: {}", session0.getCounters().getTxEnquireLink());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "tx-submitSM: {}", session0.getCounters().getTxSubmitSM());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "tx-deliverSM: {}", session0.getCounters().getTxDeliverSM());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "tx-dataSM: {}", session0.getCounters().getTxDataSM());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "rx-enquireLink: {}", session0.getCounters().getRxEnquireLink());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "rx-submitSM: {}", session0.getCounters().getRxSubmitSM());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "rx-deliverSM: {}", session0.getCounters().getRxDeliverSM());
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "rx-dataSM: {}", session0.getCounters().getRxDataSM());
            }
        ...
        Logger.getLogger(Airtelmomt.class
                .getName()
    ...
    public static class ClientSmppSessionHandler extends DefaultSmppSessionHandler {
        ...
        public void firePduRequestExpired(PduRequest pduRequest) {
            Logger.getLogger(Airtelmomt.class.getName()).log(Level.SEVERE, "PDU request expired: {}", pduRequest);
        }
        ...
        public PduResponse firePduRequestReceived(PduRequest pduRequest) {
            ...
            // do any logic here
            Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "SMS Received: {}", pduRequest);
            if (pduRequest.getCommandId() == SmppConstants.CMD_ID_DELIVER_SM) {
                ...
                Logger.getLogger(Airtelmomt.class.getName()).log(Level.INFO, "Inbox: From:" + from + " To:" + dest + " Msg:" + message);
                if (!message.contains("dlvrd")) {
                    if (!dest.equals("22252")) {
                        DataStore data = new DataStore();
                        String insert = "INSERT INTO `smsgateways`.`inbox` (`smsc`, `osmsc`, `sender`, `reciever`, `message`) VALUES ('AIRTELRX', '" + serviceT + "', '" + from + "', '" + dest + "', '" + message + "')";
                        data.insert(insert);
                    }
                }
