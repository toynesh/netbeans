    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (null != request.getParameter("self")) {
                ...
                if (userres.startsWith("found")) {
                    ...
                    if (session == null) {
                        ...
                        session.setAttribute("retailer", usrressplit[2]);
                    } else {
                        // Already created.
                        session.invalidate();
                        session = request.getSession();
            ...
            if (null != session.getAttribute("retailer")) {
                String retailer = (String) session.getAttribute("retailer");
                //System.out.println("User already Loggedin");
                if (!retailer.equals("Admin")) {
                    System.out.println("NEW USER LOGGED IN: " + session.getAttribute("retailer") + "||" + session.getAttribute("fullname"));
            ...
            out.println("<h5><i class='fas fa-address-card'></i>&nbsp;&nbsp;<u>Login</u></h5>");
            out.println("<br />");
