...
Logger log = Logger.getLogger(sms.class.getName());
    ...
    public NotifySmsDeliveryReceiptResponse notifySmsDeliveryReceipt(java.lang.String correlator, org.csapi.schema.parlayx.sms.v2_2.DeliveryInformation deliveryStatus) {
        ...
        try {
            ...
            Logger.getLogger(sms.class.getName()).log(Level.INFO, "Blast Delivery Value :" + deliveryStatus.getDeliveryStatus());
            Logger.getLogger(sms.class.getName()).log(Level.INFO, "Blast Delivery Address :" + deliveryStatus.getAddress());

        } catch (NullPointerException npe) {
            status = "NotDeliveredToTerminal";
        }
        String msisdn = deliveryStatus.getAddress().replaceAll("tel:", "");
        String insert = "insert into blast_dlr_status (msisdn,correlator,status) values ('" +msisdn+ "','" + correlator + "','" + status + "')";
        data.insert(insert);
